...
class ASquaredCPPOAgent(BaseAgent):
    ...
    def compute_pi_hat(self, prediction, prev_option, is_intial_states):
        ...
        # 
        ### torch.where is acting weird here.
        # try:
        # print(beta.size())
        pi_hat = torch.tensor(torch.where(is_intial_states.bool(), inter_pi, pi_hat))
        # except:
        # print(is_intial_states.size())
        # print(inter_pi.size())
        
        # print('by COMPUTEPIHAT')
        return pi_hat
    ...
    def learn(self, storage, mdp, freeze_v=False):
        ...
        for _ in range(config.optimization_epochs):
            ...
            for batch_indices in sampler:
                ...
                # print(sampled_states.size())
                prediction = self.network(sampled_states)

                if mdp == 'hat':
                    # print(sampled_init.size(), sampled_init.view(-1).size())
